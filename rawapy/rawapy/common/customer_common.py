...
def set_customer_code(doc,method):
    try:
        if not doc.custom_customer_code:
            last_customers=frappe.get_all("Customer",filters={"territory":doc.territory,"custom_channel":doc.custom_channel},fields=["name","custom_channel","territory","custom_territory_code","custom_channel_code","custom_customer_code"],order_by="creation desc",limit=1)
            if last_customers:
                ...
                customer_code=doc.custom_territory_code+"-"+doc.custom_channel_code
                doc.custom_customer_code=customer_code+"-"+str("1").zfill(7)
    except Exception:
        pass

def generate_customer_code():
        territories=frappe.get_all("Territory",fields=["name","custom_codes"])
        channels=frappe.get_all("Channels",fields=["name","code"])
        for territory in territories:
            if territory.name  not in ["Saudi Arabia","Rest Of The World","All Territories"]:
                for channel in channels:
                        customers=frappe.get_all("Customer",filters={"territory":territory.name,"custom_channel":channel.name},fields=["name","custom_channel","territory","custom_territory_code","custom_channel_code"],order_by="creation")
                        counter=1
